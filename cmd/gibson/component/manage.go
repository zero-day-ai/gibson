...
func runList(cmd *cobra.Command, args []string, cfg Config) error {
	ctx := cmd.Context()
	...
	// Get all components of the specified kind
...
func runShow(cmd *cobra.Command, args []string, cfg Config) error {
	...
	cmd.Printf("%s: %s\n", strings.Title(cfg.DisplayName), comp.Name)
...
func runUninstall(cmd *cobra.Command, args []string, cfg Config, flags *UninstallFlags) error {
	...
	cmd.Printf("%s '%s' uninstalled successfully in %v\n", strings.Title(cfg.DisplayName), componentName, result.Duration)
	return nil
}
...
func runUpdate(cmd *cobra.Command, args []string, cfg Config, flags *UpdateFlags) error {
	...
	if !result.Updated {
		cmd.Printf("%s '%s' is already up to date (v%s)\n", strings.Title(cfg.DisplayName), componentName, result.OldVersion)
		return nil
	}
...
func runBuild(cmd *cobra.Command, args []string, cfg Config) error {
	...
	if workDir == "" {
		workDir = comp.Path // Fallback to old Path field for backward compatibility
	}
	...
	cmd.Printf("%s '%s' built successfully in %v\n", strings.Title(cfg.DisplayName), componentName, duration)
