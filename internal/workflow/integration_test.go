...
func (m *mockHarness) DelegateToAgent(ctx context.Context, agentName string, task agent.Task) (agent.Result, error) {
	...
	if delay := m.getDelay(agentName); delay > 0 {
		select {
		case <-ctx.Done():
			return agent.Result{Status: agent.ResultStatusCancelled}, ctx.Err()
...
func (m *mockHarness) QueryFindings(ctx context.Context, filters map[string]any) ([]agent.Finding, error) {
	return nil, errors.New("not implemented in mock")
}
...
func TestIntegration_FullWorkflowLifecycle(t *testing.T) {
	...
	// Entry/exit points are computed from edges (none added), so all nodes are both entry and exit
	assert.NotEmpty(t, workflow.EntryPoints)
	assert.NotEmpty(t, workflow.ExitPoints)

	// Validate DAG
	validator := NewDAGValidator()
	err = validator.Validate(workflow)
	require.NoError(t, err, "workflow should be valid")
...
func TestIntegration_RetryPolicy_ExhaustedRetries(t *testing.T) {
	...
	// When a node fails after retries, it may not have a NodeResult in the results map
	// but the NodeState should still be tracked
	assert.Equal(t, 0, result.NodesExecuted, "no nodes should complete successfully")
	assert.Equal(t, 1, result.NodesFailed, "one node should have failed")

	// The node result may or may not exist depending on implementation
	// What matters is that the workflow tracked the failure
}
...
func TestIntegration_FailurePolicy_NodeFailure(t *testing.T) {
	...
	// Workflow ends in deadlock because step3 can't execute (step2 failed)
	require.Error(t, err) // Expect deadlock error
	...
	if step1Result, exists := result.NodeResults["step1"]; exists {
		assert.Equal(t, NodeStatusCompleted, step1Result.Status)
	}
	if step2Result, exists := result.NodeResults["step2"]; exists {
		assert.Equal(t, NodeStatusFailed, step2Result.Status)
	}
	// step3 won't execute because step2 failed (deadlock)
}
...
func TestIntegration_DiamondDependencies(t *testing.T) {
	...
	// Entry/exit points computed from edges (dependencies don't create edges in builder)
	assert.NotEmpty(t, workflow.EntryPoints)
	assert.NotEmpty(t, workflow.ExitPoints)
...
func TestIntegration_JoinNodeAggregation(t *testing.T) {
	...
	aggregatedOutput := joinResult.Output
	require.NotNil(t, aggregatedOutput, "join output should not be nil")
...
func TestIntegration_JoinNodeWithFailedDependency(t *testing.T) {
	...
	// Workflow completes but with deadlock because join can't execute (dependency failed)
	require.Error(t, err)
	assert.Equal(t, WorkflowStatusFailed, result.Status)
	assert.Equal(t, 1, result.NodesExecuted) // Only A succeeded
	assert.Equal(t, 1, result.NodesFailed)   // B failed

	// Join node never executes because dependency B failed
	joinResult := result.NodeResults["join"]
	assert.Nil(t, joinResult, "join should not execute if dependency fails")
}
...
func TestIntegration_ParallelNodeWithFailedSubNode(t *testing.T) {
	...
	// Parallel node should have failed
	// Failed nodes don't count as "executed" - only completed nodes do
	assert.Equal(t, 0, result.NodesExecuted, "failed nodes aren't counted as executed")
	assert.Equal(t, 1, result.NodesFailed, "one node failed")

	// Check if result exists
	if parallelResult, exists := result.NodeResults["parallel"]; exists {
		...
		if parallelResult.Error != nil {
			...
			if parallelResult.Error.Details != nil {
...
func TestIntegration_ComplexMultiStageWorkflow(t *testing.T) {
	...
	workflow, err := NewWorkflow("complex-workflow").
		...
		AddToolNode("port_scan", "portScanner", nil).
		AddToolNode("vuln_scan", "vulnScanner", nil).
		AddToolNode("web_scan", "webScanner", nil).
		// Stage 2: Condition based on scan results
		AddConditionNode("check_vulns", &NodeCondition{
			Expression:  "nodes.vuln_scan.output.vulnerabilities_found > 0",
		...
		AddNode(&WorkflowNode{
			...
			Dependencies: []string{"port_scan", "vuln_scan", "web_scan"},
		}).
		// Stage 4: Final report
		AddToolNode("report", "reportGenerator", nil).
		WithDependency("check_vulns", "vuln_scan").
		WithDependency("analyze", "check_vulns").
		WithDependency("skip", "check_vulns").
		WithDependency("report", "join_scans", "analyze", "skip").
		Build()
